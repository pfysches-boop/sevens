/** // TODO //
 * player profile pictures
 */

import { Socket } from "socket.io"
// @ts-expect-error
const socket: Socket = io()

const cardBack = '🂠'
const animationDuration_ms: number = parseInt((
  Array.from(document.styleSheets[0].cssRules)
    .filter((rule: CSSRule) => (rule as CSSStyleRule).selectorText === '.moving')[0] as 
  CSSStyleRule).style.animationDuration
)

enum SortingMode {
  None, ByRank, BySuit,
}
let sortingMode: SortingMode = SortingMode.None

// socket listenrs
let userCount: number = 0
socket.on('userCount', (count: number) => {
  userCount = count
})

socket.on('tenScore_in', (newTenScore: number) => {
  tenScore = newTenScore
})

socket.on('direction_in', (newDirection: Direction) => {
  state.direction = newDirection
})

let names: string[] = []
socket.on('names_in', (newNames: string[]) => {
  names = [...newNames]
  updateOpponentHandDiv()
})

let socketIndex: number = -1
let state: GameState = {
  playerCount: 0,
  
  scores: [],
  roundsPlayed: 0,
  
  hands: [],
  table: {},
  
  lastTurnIndex: -1,
  turnIndex: -1,
  socketIndex: -1,
  
  requestingCard: false,
  givingCard: false,

  cardPlayed: null,

  gameStart: false,
  gameOver: false,

  direction: Direction.Clockwise,
}
socket.on('state_in', (stateString: string) => {
  state = parseStateString(stateString)

  if(state.socketIndex !== -1) {
    socketIndex = state.socketIndex
  }

  if(state.turnIndex === socketIndex) {
    handleClientTurn()
  } else {
    handleOpponentTurn()
  }

  
  if(state.gameOver) {
    handleGameOver()
  }
})



// HTML elements
const root: HTMLHtmlElement = document.querySelector(':root') as HTMLHtmlElement

const startGameMenuItem      = document.getElementById('start-game-menuitem')      as
  HTMLAnchorElement
const viewScoreboardMenuItem = document.getElementById('view-scoreboard-menuitem') as 
  HTMLAnchorElement
const editRulesMenuItem      = document.getElementById('edit-rules-menuitem')      as
  HTMLAnchorElement
const editProfileMenuitem    = document.getElementById('edit-profile-menuitem')    as
  HTMLAnchorElement

const opponentHandDiv     = document.getElementById('opponent-hand-div') as
  HTMLDivElement
const opponentHandOverlay = document.getElementById('opponent-hand-overlay') as
  HTMLDivElement

const tableDiv     = document.getElementById('table-div') as
  HTMLDivElement
const tableOverlay = document.getElementById('table-overlay') as
  HTMLDivElement
const tableTable   = document.getElementById('table-table') as
  HTMLTableElement

const sortByRankMenuItem  = document.getElementById('sort-by-rank-menuitem') as
  HTMLAnchorElement
const sortBySuitMenuItem  = document.getElementById('sort-by-suit-menuitem') as
  HTMLAnchorElement
const requestCardMenuItem = document.getElementById('request-card-menuitem') as
  HTMLAnchorElement

const clientHandDiv     = document.getElementById('client-hand-div') as
  HTMLDivElement
const clientHandOverlay = document.getElementById('client-hand-overlay') as
  HTMLDivElement

const divWidth: number = clientHandDiv.clientWidth
const cardsPerRow: number = 7
const cardWidth: number = (Math.floor(divWidth / cardsPerRow) - (cardsPerRow - 1)) * 0.8
const cardHeight: number = Math.floor(cardWidth * 2)

// HTML listeners
startGameMenuItem.onclick = (evt: PointerEvent) => { startGame() }

viewScoreboardMenuItem.onclick = (evt: PointerEvent) => {
  if(
    isOpponentHandOverlayVisible() && 
    !isRulesOverlayOpen() &&
    !isProfileOverlayOpen()
  ) {
    hideOpponentHandOverlay()
    closeScoreOverlay()
    return
  }

  closeRulesOverlay()
  closeProfileOverlay()

  opponentHandOverlay.style.visibility = ''
  viewScoreboardMenuItem.className = 'special'
  viewScoreboardMenuItem.innerHTML = 'Close Scoreboard'
  
  opponentHandOverlay.replaceChildren(makeScoreTable())
}
editRulesMenuItem.onclick = (evt: PointerEvent) => {
  if(
    isOpponentHandOverlayVisible() && 
    !isScoreOverlayOpen() &&
    !isProfileOverlayOpen()
  ) {
    hideOpponentHandOverlay()
    closeRulesOverlay()
    return
  }

  closeScoreOverlay()
  closeProfileOverlay()

  opponentHandOverlay.style.visibility = ''
  editRulesMenuItem.className = 'special'
  editRulesMenuItem.innerHTML = 'Close Game Rules'

  opponentHandOverlay.replaceChildren(makeRulesForm())
}
editProfileMenuitem.onclick = (evt: PointerEvent) => {
  if(
    isOpponentHandOverlayVisible() &&
    !isScoreOverlayOpen() &&
    !isRulesOverlayOpen()
  ) {
    hideOpponentHandOverlay()
    closeProfileOverlay()
    return
  }

  closeScoreOverlay()
  closeRulesOverlay()

  
  opponentHandOverlay.style.visibility = ''
  editProfileMenuitem.className = 'special'
  editProfileMenuitem.innerHTML = 'Close Profile'

  opponentHandOverlay.replaceChildren(makeProfileForm())
}

sortByRankMenuItem.onclick = (evt: PointerEvent) => {
  sortingMode = SortingMode.ByRank
  updateClientHandDiv()
}
sortBySuitMenuItem.onclick = (evt: PointerEvent) => {
  sortingMode = SortingMode.BySuit
  updateClientHandDiv()
}

function hideOpponentHandOverlay(): void {
  opponentHandOverlay.style.visibility = 'hidden'
}
function closeScoreOverlay(): void {
  viewScoreboardMenuItem.innerHTML = 'View Scoreboard'
  viewScoreboardMenuItem.className = ''
}
function closeRulesOverlay(): void {
  editRulesMenuItem.innerHTML = 'Edit Game Rules'
  editRulesMenuItem.className = ''
}
function closeProfileOverlay(): void {
  editProfileMenuitem.innerHTML = 'Edit Profile'
  editProfileMenuitem.className = ''
}

function isOpponentHandOverlayVisible(): boolean {
  return opponentHandOverlay.style.visibility !== 'hidden'
}
function isScoreOverlayOpen(): boolean {
  return viewScoreboardMenuItem.classList.contains('special')
}
function isRulesOverlayOpen(): boolean {
  return editRulesMenuItem.classList.contains('special')
}
function isProfileOverlayOpen():  boolean {
  return editProfileMenuitem.classList.contains('special')
}

function getNextTurnIndex() {
  return (
    state.direction === 'cw' ? (state.turnIndex + 1) % state.playerCount :
      (state.turnIndex + state.playerCount - 1) % state.playerCount
  )
}
function getLastTurnIndex() {
  return (
    state.direction === 'cw' ? (state.turnIndex + state.playerCount - 1) % state.playerCount :
      (state.turnIndex + 1) % state.playerCount
  )
}

function isFirstTurn():            boolean {
  return state.lastTurnIndex === -1
}
function isClientTurn():           boolean {
  return state.turnIndex === socketIndex && socketIndex !== -1
}
function wasClientTurn():          boolean {
  return state.lastTurnIndex === socketIndex && socketIndex !== -1
}
function cardJustGiven():          boolean { 
  return state.lastTurnIndex === state.turnIndex
}
function clientJustGaveCard():     boolean {
  return cardJustGiven() &&
    state.turnIndex === socketIndex && 
    socketIndex !== -1
}
function clientJustReceivedCard(): boolean {
  return cardJustGiven() &&
    getLastTurnIndex() === socketIndex &&
    socketIndex !== -1
}

function getMovingElementsCount(): number {
  return document.getElementsByClassName('moving').length
}

type CardElement = { card: Card | null, element: HTMLAnchorElement }
function makeCardElement(card: Card | null): CardElement {
  const element: HTMLAnchorElement = document.createElement('a')

  element.style = `
    padding: 0px;
    font-size: ${cardHeight * 0.8}px;
    margin: 0px 1px;
    cursor: pointer;
    background-color: white;
  `

  if(card !== null) {
    element.innerHTML = card.toString()
    if(!isCardPlayable(card)) {
      element.className = 'unplayable'
      element.style.color =
        card.suit === CardSuit.Diamonds || card.suit === CardSuit.Hearts
          ? 'lightpink'
        : 'lightgray'
    } else {
      element.style.color = 
        card.suit === CardSuit.Diamonds || card.suit === CardSuit.Hearts
          ? 'red' 
        : 'black'
    }
  } else {
    element.style.color = '#2200dd'
    element.innerHTML = cardBack
    element.onclick = null
  }

  if(!isCardPlayable(card)) {
    element.style.cursor = ''
  }

  return { card: card, element: element }
}

const cardStrings: string[] = [ 
  cardBack,
  ...Card.getDeck()
    .map((card: Card) => card.toString())
]
function isCardElement(element: Element): boolean {
  return cardStrings.includes(element.innerHTML)
}

function makeScoreTable(): HTMLTableElement {
  const table = document.createElement('table')

  const headerRow = document.createElement('tr')
  const labelHeaderCell = document.createElement('td')
  headerRow.appendChild(labelHeaderCell)
  for(let i = 0; i < state.playerCount; i++) {
    const cell = document.createElement('td')
    cell.innerHTML = `${names[i]}`
    headerRow.appendChild(cell)
  }
  table.appendChild(headerRow)

  for(let i = 0; i < state.scores.length; i++) {
    const row       = document.createElement('tr')
    const labelCell = document.createElement('td')
    labelCell.innerHTML = `[${1 + i}]`
    row.append(labelCell)
    const minScore: number = Math.min(...state.scores[i])
    for(const score of state.scores[i]) {
      const cell = document.createElement('td')
      cell.innerHTML = `${score}`
      
      if(i === 6 && score === minScore)
        cell.className = 'special'

      row.append(cell)
    }
    table.append(row)
  }

  return table
}
function makeRulesForm(): HTMLFormElement {
  const form = document.createElement('form')

  form.innerHTML = `
    <fieldset>
      <legend>Score value of 10s</legend>

      <div>
        <input type="radio" id="ten-score-5"  name="ten-score" value="5" ${tenScore === 5 ? 'checked="true"' : ''} />
        <label for="ten-score-5">5</label>

        <input type="radio" id="ten-score-10" name="ten-score" value="10" ${tenScore === 10 ? 'checked="true"' : ''} />
        <label for="ten-score-10">10</label>
      </div>
    </fieldset>

    <fieldset>
      <legend>Turn direction</legend>

      <div>
        <input type="radio" id="direction-cw"  name="direction" value="cw" ${state.direction === 'cw' ? 'checked="true"' : ''} />
        <label for="direction-cw">Clockwise</label>

        <input type="radio" id="direction-ccw" name="direction" value="ccw" ${state.direction === 'ccw' ? 'checked="true"' : ''} />
        <label for="direction-ccw">Counter-Clockwise</label>
      </div>
    </fieldset>
  `


  form.onchange = (evt: Event) => {
    const data = new FormData(form)
    
    const newTenScore:  number   = +(data.get('ten-score') as FormDataEntryValue).toString()
    const newDirection: Direction = (data.get('direction') as FormDataEntryValue).toString() as Direction


    socket.emit('tenScore_out',  newTenScore)
    socket.emit('direction_out', newDirection)
  }

  return form
}
function makeProfileForm(): HTMLDivElement {
  const form: HTMLDivElement = document.createElement('div')

  form.innerHTML = `
    <div>
      <label for="profile-name-input">Name</label>
      <input type="text" id="profile-name-input" name="profile" placeholder="Player ${1 + socketIndex}" />
    </div>
  `

  form.onchange = (evt: Event) => {
    const input: HTMLInputElement = 
      document.getElementById('profile-name-input') as
        HTMLInputElement
    const name: string = input.value || input.placeholder
    names[socketIndex] = name
    sendNames()
  }

  return form
}

function updateOpponentHandDiv(): void {
  opponentHandDiv.replaceChildren(opponentHandOverlay)

  for(let i = 0; i < state.playerCount; i++) {
    if(i === socketIndex) continue
    const element: CardElement = makeCardElement(null)
    const countLabel: HTMLAnchorElement = document.createElement('a')
    countLabel.style.paddingTop = '3vh'
    countLabel.style.whiteSpace = 'pre-line'
    countLabel.innerHTML = `${names[i]}\nx${state.hands[i].length}`
    opponentHandDiv.append(element.element, countLabel)
  }
}
function updateClientHandDiv(): void {
  clientHandDiv.replaceChildren(clientHandOverlay)
  sortCards(state.hands[socketIndex])

  for(const card of state.hands[socketIndex]) {
    const cardElement = makeCardElement(card)
    if(cardElement.element.className !== 'unplayable') {
      cardElement.element.onclick = (evt: PointerEvent) => {
        handleCardElementPress(cardElement)
      }
    }
    clientHandDiv.appendChild(cardElement.element)
  }
}
function updateTableDiv(): void {
  for(const key of Object.keys(state.table)) {
    const pile: Card[] = state.table[key]
    const highCell = document.getElementById(`${key}-high-cell`) as
      HTMLTableCellElement
    const lowCell = document.getElementById(`${key}-low-cell`) as
      HTMLTableCellElement

    if(pile.length === 0) {
      highCell.replaceChildren(makeCardElement(null).element)
      lowCell.replaceChildren(makeCardElement(null).element)
      continue
    }

    const sortedPile: Card[] = pile.toSorted(
      (a: Card, b: Card) => cardRankToNumber[a.rank] - cardRankToNumber[b.rank]
    )
    const highCard: Card = sortedPile[sortedPile.length - 1]
    const lowCard:  Card = sortedPile[0]

    lowCell.replaceChildren(makeCardElement(lowCard).element)
    if(pile.length > 1) {
      highCell.replaceChildren(makeCardElement(highCard).element)
    }
  }
}

function updateHTML(): void {
  updateOpponentHandDiv()
  updateClientHandDiv()
  animateLastTurn()
  
  setTimeout(
    updateTableDiv,
    wasClientTurn() || isFirstTurn() ? 0 : animationDuration_ms
  )
}


function setAnimationEndPoint(start: DOMRect, end: DOMRect) {
  root.style.setProperty('--end-position-top',  `${end.top  - start.top }px`)
  root.style.setProperty('--end-position-left', `${end.left - start.left}px`)
}


function animateGhostCard(
  card: Card | null,
  sourceDiv: HTMLDivElement,
  sourceChildArray: Element[],
  sourceHandAnchors: HTMLAnchorElement[],
  index: number
) {
  const ghostCardElement = makeCardElement(card)
  const replacementIndex: number = 
    sourceChildArray.findIndex(
      (element: Element) => element === sourceHandAnchors[index]
    )
  const replacedChild = sourceChildArray[replacementIndex]


  sourceDiv.replaceChildren(
    ...sourceChildArray.slice(0, replacementIndex),
    ghostCardElement.element,
    ...sourceChildArray.slice(replacementIndex + 1)
  )

  ghostCardElement.element.className = 'moving'
  setTimeout(() => {
    sourceDiv.replaceChildren(
      ...sourceChildArray.slice(0, replacementIndex),
      replacedChild,
      ...sourceChildArray.slice(replacementIndex + 1)
    )
  }, animationDuration_ms)
}

function animateOpponentPlayedCard(
  card: Card,
  opponentChildArray: Element[],
  opponentHandAnchors: HTMLAnchorElement[],
  opponentIndex: number
): void {
  const startRect: DOMRect = 
    opponentHandAnchors[opponentIndex].getBoundingClientRect()
  const endRect:   DOMRect =
    getTableDestinationAnchor(card).getBoundingClientRect()
  setAnimationEndPoint(startRect, endRect)

  animateGhostCard(
    card,
    opponentHandDiv,
    opponentChildArray,
    opponentHandAnchors,
    opponentIndex
  )
}
function animateClientReceivedCard(
  card: Card,
  opponentChildArray: Element[],
  opponentHandAnchors: HTMLAnchorElement[],
  opponentIndex: number
): void {
  const startRect: DOMRect = 
    opponentHandAnchors[opponentIndex].getBoundingClientRect()
  
  const cardElementInHand: HTMLAnchorElement =
    Array.from(clientHandDiv.children).find(
      (element: Element) => element.innerHTML === card.toString()
    ) as HTMLAnchorElement
  const endRect:   DOMRect =
    cardElementInHand.getBoundingClientRect()
  setAnimationEndPoint(startRect, endRect)

  cardElementInHand.style.visibility = 'hidden'
  animateGhostCard(
    card, 
    opponentHandDiv,
    opponentChildArray,
    opponentHandAnchors,
    opponentIndex
  )
  setTimeout(() => {
    cardElementInHand.style.visibility = ''
  }, animationDuration_ms);
}
function animateOpponentGaveCard(
  opponentChildArray: Element[],
  opponentHandAnchors: HTMLAnchorElement[],
  opponentIndex: number
): void {
  const startRect: DOMRect = 
    opponentHandAnchors[opponentIndex].getBoundingClientRect()
  
  let otherIndex: number = getLastTurnIndex()
  if(otherIndex > socketIndex) otherIndex--
  const endRect:   DOMRect =
    opponentHandAnchors[otherIndex].getBoundingClientRect()
    
  setAnimationEndPoint(startRect, endRect)

  animateGhostCard(
    null,
    opponentHandDiv,
    opponentChildArray,
    opponentHandAnchors,
    opponentIndex
  )
}

function animateLastTurn(): void {
  if(state.playerCount === 1) return
  if(state.gameOver) return
  if(isFirstTurn() || wasClientTurn() || state.cardPlayed === null) return
  
  const card: Card = state.cardPlayed as Card
  const opponentHandAnchors: HTMLAnchorElement[] = 
    Array.from(opponentHandDiv.children).filter(
      (element: Element) => isCardElement(element)
    ) as HTMLAnchorElement[]
  const opponentChildArray: Element[] = Array.from(opponentHandDiv.children)
  let opponentIndex: number = isClientTurn() ? getLastTurnIndex() : state.turnIndex
  if(opponentIndex > socketIndex) opponentIndex--


  if(clientJustReceivedCard()) {
    animateClientReceivedCard(
      card,
      opponentChildArray,
      opponentHandAnchors,
      opponentIndex
    )
    return
  }

  if(cardJustGiven()) {
    animateOpponentGaveCard(
      opponentChildArray,
      opponentHandAnchors,
      opponentIndex
    )
    return
  }

  animateOpponentPlayedCard(
    card,
    opponentChildArray,
    opponentHandAnchors,
    opponentIndex
  )
}

function sortCards(cards: Card[]): void {
  cards.sort(
    (a: Card, b: Card) => {
      const rankDiff: number = cardRankToNumber[a.rank] - cardRankToNumber[b.rank]
      const suitDiff: number = cardSuitToNumber[a.suit] - cardSuitToNumber[b.suit]

      return (
        sortingMode === SortingMode.ByRank ? rankDiff || suitDiff :
        sortingMode === SortingMode.BySuit ? suitDiff || rankDiff :
        0
      )
    }
  )
}

function isCardPlayable(card: Card | null): boolean {
  if(card === null) return false

  const previousCardPlayed: boolean = state.table[card.suit].filter(
    (value: Card) => value.rank === previousCardRank(card.rank)
  ).length !== 0
  const nextCardPlayed: boolean = state.table[card.suit].filter(
    (value: Card) => value.rank === nextCardRank(card.rank)
  ).length !== 0

  if(card.rank !== CardRank.Seven && !previousCardPlayed && !nextCardPlayed)
    return false

  const spadePlayed: boolean = state.table[CardSuit.Spades].filter(
    (value: Card) => value.rank === card.rank
  ).length !== 0

  return card.suit === CardSuit.Spades || spadePlayed
}


function getTableDestinationAnchor(card: Card): HTMLAnchorElement {
  const pile: Card[] = state.table[card.suit]

  if(pile.length === 0)
    return document.getElementById(`${card.suit}-low-cell`)?.children[0] as HTMLAnchorElement

  const sortedPile: Card[] = pile.toSorted(
    (a: Card, b: Card) => cardRankToNumber[a.rank] - cardRankToNumber[b.rank]
  )
  const highCard: Card = sortedPile[sortedPile.length - 1]

  if(cardRankToNumber[highCard.rank] < cardRankToNumber[card.rank])
    return document.getElementById(`${card.suit}-high-cell`)?.children[0] as HTMLAnchorElement
  
  return document.getElementById(`${card.suit}-low-cell`)?.children[0] as HTMLAnchorElement
}

function handleCardElementPress(cardElement: CardElement | null): void {
  if(
    socketIndex !== state.turnIndex ||
    cardElement === null ||
    cardElement.card === null ||
    !isCardPlayable(cardElement.card)
  ) return

  if(getMovingElementsCount() > 0) return

  const card: Card = cardElement.card as Card
  const cardIndex: number = state.hands[socketIndex].findIndex(
    (value: Card) => value.equals(card)
  )
  if(cardIndex === -1) return


  const startRect: DOMRect = cardElement.element.getBoundingClientRect()
  const endRect: DOMRect = getTableDestinationAnchor(card).getBoundingClientRect()

  setAnimationEndPoint(startRect, endRect)
  cardElement.element.className = 'moving'

  setTimeout(() => {
    state.cardPlayed = card

    state.table[card.suit].push(card)
    
    state.hands[socketIndex].splice(cardIndex, 1)
    if(state.hands[socketIndex].length === 0) endGame()
    sendState()

  }, animationDuration_ms)
}


function handleClientTurn(): void {
  updateHTML()
  clientHandOverlay.style.visibility = 'hidden'

  if(Array.from(clientHandDiv.children).filter(
    (element: Element) => !element.classList.contains('unplayable')
      && !element.classList.contains('overlay')
  ).length === 0 && !state.gameOver) {
    enableCardRequest()
  }

  if(state.givingCard) {
    enableCardGive()
  }
}
function enableCardRequest(): void {
  requestCardMenuItem.style.visibility = ''
  requestCardMenuItem.onclick = (evt: PointerEvent) => {

    requestCardMenuItem.style.visibility = 'hidden'
    state.requestingCard = true
    state.cardPlayed = null
    sendState()
  }
}
function enableCardGive(): void {
  tableOverlay.style.visibility = ''
  tableOverlay.innerHTML = 
    `${names[state.lastTurnIndex]} is asking for a card!<br/>` +
    `Please click the card you want to give them`

  for(let i = 0; i < state.hands[socketIndex].length; i++) {
    const card: Card = state.hands[socketIndex][i]
    const anchor = clientHandDiv.children[1 + i] as HTMLAnchorElement

    anchor.onclick = (evt: PointerEvent) => {
      if(getMovingElementsCount() > 0) return

      const startRect: DOMRect =
        anchor.getBoundingClientRect()

      let opponentIndex = getLastTurnIndex()
      if(opponentIndex > socketIndex) opponentIndex--
      const endRect:   DOMRect =
        Array.from(opponentHandDiv.children).filter(
          (element: Element) => isCardElement(element)
        )[opponentIndex].getBoundingClientRect()
      
      setAnimationEndPoint(startRect, endRect)

      anchor.classList.add('moving')
      setTimeout(() => {
        tableOverlay.style.visibility = 'hidden'

        state.cardPlayed = card
        state.hands[socketIndex].splice(i, 1)
        state.hands[state.lastTurnIndex].push(card)
        sendState()

        if(state.hands[socketIndex].length === 0) endGame()
      }, animationDuration_ms)
    }
  }
  
}

function handleOpponentTurn(): void {
  updateHTML()
  clientHandOverlay.style.visibility = ''
  clientHandOverlay.innerHTML =
    state.givingCard ? 
      `${names[state.turnIndex]} is selecting a card to give to ${names[state.lastTurnIndex]}...`
    : `It's ${names[state.turnIndex]}'s turn...`
}


function handleGameOver(): void {
  const winnerIndex: number =
    state.scores.map((q: number[], i: number) => ({ index: i, value: q }))
      .toSorted((a, b) => a.value[a.value.length - 1] - b.value[b.value.length - 1])[0]
      .index

  clientHandOverlay.style.visibility = ''
  clientHandOverlay.innerHTML =
    `Game over!<br/>` +
    `${names[winnerIndex]} wins!`

  opponentHandOverlay.style.visibility = 'hidden'
  viewScoreboardMenuItem.click()

  state.gameOver = false
}


function sendState(): void {
  socket.emit('state_out', JSON.stringify(state))
}
function sendNames(): void {
  socket.emit('names_out', names)
}


function startGame(): void {
  if(state.roundsPlayed >= 7) {
    state.roundsPlayed = 0
    state.scores = []
  }
  
  state.playerCount = userCount
  for(let i = 0; i < state.playerCount; i++)
    if(!names[i]) names[i] = `Player ${1 + i}`
  sendNames()
  
  state.socketIndex = -1
  state.turnIndex = -1

  const deck: Card[] = Card.getDeck()
  deck.sort((a: Card, b: Card) => Math.random() < 0.5 ? -1 : 1)

  const hands: Card[][] = []
  for(let i = 0; i < state.playerCount; i++) hands[i] = []
  while(deck.length > 0) {
    for(let i = 0; i < state.playerCount && deck.length > 0; i++) {
      hands[i].push(deck[0])
      deck.splice(0, 1)
    }
  }

  state.hands = [...hands]

  state.table = {}
  state.table[CardSuit.Spades  ] = []
  state.table[CardSuit.Diamonds] = []
  state.table[CardSuit.Clubs   ] = []
  state.table[CardSuit.Hearts  ] = []

  state.gameStart = true
  sendState()
}

function endGame(): void {
  for(let i = 0; i < state.playerCount; i++) {
    if(!state.scores[state.roundsPlayed]) state.scores[state.roundsPlayed] = []
    if(!state.scores[state.roundsPlayed][i]) { state.scores[state.roundsPlayed][i] = 0 }
    
    const lastScore: number = state.roundsPlayed > 0 ? state.scores[state.roundsPlayed - 1][i] : 0
    const score: number = state.hands[i].reduce((t: number, c: Card) => t + c.score, 0)
    state.scores[state.roundsPlayed][i] = lastScore + score
  }
  state.roundsPlayed++
  
  if(state.roundsPlayed < 7) startGame()
  else state.gameOver = true
}